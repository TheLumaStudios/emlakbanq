...
export default function Settings() {
  ...
  // Sync whatsapp number from site settings
  useEffect(() => {
    const wp = siteSettings.find((s) => s.key === 'whatsapp')
    if (wp) setWhatsappNumber(typeof wp.value === 'string' ? wp.value : String(wp.value || ''))
  }, [siteSettings])
  ...
  async function saveWhatsappNumber() {
    ...
    const existing = siteSettings.find((s) => s.key === 'whatsapp')

    let result
    if (existing) {
      result = await supabase
        .from('site_settings')
        .update({ value: whatsappNumber, updated_at: new Date().toISOString() })
        .eq('id', existing.id)
    } else {
      result = await supabase
        .from('site_settings')
        .insert([{ key: 'whatsapp', value: whatsappNumber, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }])
    }
            ...
            <div className="px-5 py-5">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-estate-700">{t('admin.settings.whatsappNumber', 'WhatsApp Numarası')}</span>
                </div>
                <button
              ...
              <input
                ...
                className="w-full rounded-lg border border-estate-200 bg-white px-3 py-2 text-sm text-estate-900 placeholder:text-estate-300 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:max-w-xs"
              />
              <p className="mt-1.5 text-xs text-estate-400">
                {t('admin.settings.whatsappHint', 'Ülke kodu ile birlikte girin (örn: 905551234567)')}
              </p>
            </div>
